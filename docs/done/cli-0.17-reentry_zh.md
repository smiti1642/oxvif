...
| --- | --- |
| [實作批次](#實作批次) | 範圍及驗證 |
| [搜尋返回後續修正](#搜尋返回後續修正) | 工作階段快取及針對性回歸證據 |
...
本節記錄初次 Back 修正；後續[工作流程連續性批次](cli-workflow-continuity_zh.md)
已將此處的獨立 setup 退出行為改成連續同畫面新增，並加入各設備狀態保存。
其針對性證據與下方歷史計數分開記錄。

2026-09-13 已在舊執行檔重現返回／重新進入問題：從探索到的攝影機返回時遺失
瀏覽畫面，重新進入搜尋則再次掃描網路。現在由 `manage` 保留本次工作階段的
探索結果及檢視狀態；返回時還原兩者，大寫 `R` 才要求新掃描。成功掃描會重設
檢視，取消或失敗則保留原狀。快取有明確標示、可能非最新狀態，退出 `manage`
即清除。獨立 `discover` 在詳情／取消新增時保留狀態，但提交新增即結束指令；
重新執行仍會重新掃描。

針對性驗證通過：`cargo test -p oxvif-cli`、CLI 全 target Clippy（警告視為
錯誤），以及更新後 debug 執行檔的 Windows ConPTY 測試。終端測試比對攝影機
返回、設備選單重新進入、取消重掃後的完整篩選／捲動畫面，另確認明確重掃成功
後重設檢視、退出碼為零及隔離設備清單未變更。僅執行唯讀區域網路探索，未執行
攝影機操作或使用憑證。新增兩項單元測試涵蓋狀態還原及依畫面情境處理 `R`。
本次未重跑或取代下列歷史 workspace 計數；跨平台 CI 與發布核准仍待完成。
...
[基礎 Fleet 計畫](../active/mock-fleet-basic-plan_zh.md) 以單一程序、同 IP 不同埠為實作預設；
