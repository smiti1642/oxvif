...
探索清單依終端可用高度顯示，不再限制每頁最多 12 筆。放大／縮小視窗會調整
可見筆數，保留選取紀錄可見及底部提示／狀態列。整頁／半頁移動依目前容量計算；
符合條件的紀錄不足時，剩餘區域自然留白。
...
按 `?` 後，以 Tab 在行號設定及來源畫面的按鍵說明之間切換。說明區分一般選單、
已存設備搜尋、獨立／manage 探索、探索詳情及報告。頂部並列 **Line numbers**
與 **Read key bindings** 兩個頁籤，以方括號標示目前頁面。可用 j/k、Page Up/Down、
Ctrl+D/U 或 gg/G 捲動說明；Tab 返回行號設定，Esc/q 關閉且不套用預覽。
