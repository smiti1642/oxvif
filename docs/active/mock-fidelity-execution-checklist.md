...
| W20 / M5 / TODO | W04/W05 corpus | `tests/mock_schema_shape.rs`, dispatch's namespace tests: correct scoped resolution, Fault inclusion, coverage accounting | Independent generic prefix-shadowing controls; qualified attributes and QName-valued text; unanchored/opaque/wildcard/unresolved coverage explicit; never just relax pins |
| W21 / M5 / TODO | W20, D3 | Proposed external manifest/catalogue/fetch/validator scripts outside runtime crate | Select and record validator version after evaluating representative imports; pinned source closure/hashes, offline resolution, envelope plus payload/Fault validation; malformed corpus must fail |
| W22 / M5 / PARTIAL | W00 for inventory; W21 for schema job | `.github/workflows/ci.yml`: Windows/Linux inventory job wired, package depends on it; hosted run unverified. Dedicated schema job and release evidence check still pending | Inventory/self-tests must pass; schema job fails on missing tools/files, digest/download/import errors; least privilege, no schema/secret artifacts; missing/skipped required job blocks release acceptance |
