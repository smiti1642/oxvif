...
| W19 / M3,M6 / PARTIAL | W03/W09 designs | Built-in profile creation/deletion, Media1 video binding and Media2 generic binding use private committed effects; cross-service reads, HTTP, instance and chain controls | Remaining configuration writes and other mutations, standalone replay policy, full read dependencies, normalization/key collisions and concurrent/callback visibility remain open; no new recording of device secrets; PA1 extends committed profile effects to reference-count and PTZ-compatible reads; VS1 adds post-commit source/profile/options invalidation while physical-source and static encoder-instance recordings remain intact |
...
| W21 / M5 / PARTIAL | W20, D3 | Pinned offline tooling, 20 schema-free controls, seven independent-backend tests, and selected 24-operation profile/source/rate corpus export with explicit payload anchors | 84 instances pass locally after K34, including 12 refusals (seven profile, three source, two rate). Remaining operations and broader input/semantic coverage are not accepted |
| W22 / M5 / PARTIAL | W00 for inventory; W21 for schema job | Windows/Linux inventory, Xerces qualification, official-source compilation and selected profile/source/rate-instance validation gate package | Sources and corpus stay external with no uploaded artifacts. Selected corpus has 84 instances over 24 operations; K34 hosted CI, whole-program instance coverage and release evidence check remain pending |
...
Current progress: W00 is complete; 231 direct readers remain indexed, not accepted.
K34's approved rate migration is tracked in the [VE1 plan](mock-fidelity-video-encoder.md).
Remaining VE1 work covers selectors, options, full candidates, codec views and capacity;
then audio/metadata. K34 does not close W10 or the overall programme.
...
are implemented subgroups. **Next: W10 eight encoder configuration/options/instances operations**,
