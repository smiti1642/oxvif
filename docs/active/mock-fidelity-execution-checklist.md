...
| W18 / M4 / TODO | W10–W16 candidate behavior | `state.rs::modify/modify_returning/notify`, shared collections, event queues and replay invalidation | Concurrent invalid/valid writes, independent instances, complete rollback, hook notification count/order and reentrancy/lock behavior; persistence hook boundaries documented |
| W19 / M3,M6 / TODO | W03/W09 designs | `canon.rs`, `metamorph/{fixture,replay,record,parse,adapter,quirk,surface}.rs`, responder chain | Recorded deviations preserved; normalization/key collisions and parameter discrimination reviewed; failed writes do not wrongly invalidate replay; no new recording of device secrets |
...
| W21 / M5 / PARTIAL | W20, D3 | 23-file URL/hash catalogue, hash-pinned verification wheels, offline closure/compiler/instance tooling and 15 generic controls delivered | Closure check passed; K21 blocks full compilation. Mock corpus/export and full SOAP/payload/Fault qualification remain required; generic tests and compilation are not conformance acceptance |
| W22 / M5 / PARTIAL | W00 for inventory; W21 for schema job | Inventory passed hosted CI; Windows/Linux generic schema-tooling controls now also gate package. Full external-instance job and release evidence check remain pending | Tool controls use only project-authored schemas; they are not the official-source gate. Full gate must fail on missing tools/files, digest/download/import errors, with no schema/secret artifacts |
...
| K03 / source-confirmed | Fault builder remains flat; client exposes one subcode | W05/W06; structured wire plus compatibility evidence |
...
| K06 / source-confirmed behavior, conformance unchecked | HTTP handler returns 200; Events routes use substring matching; HTTP body uses lossy UTF-8 conversion | W03/W07; independent protocol/transport cases and applicable references |
...
| K17 / reproduced, not fixed | A rejected DeleteProfile retires the recorded GetProfile despite complete state equality | W19/W03/W18; convert the known-gap assertion to recording preservation after outcome-based invalidation; see pipeline preflight |
...
| K21 / reproduced external qualification blocker | Full XSD 1.1 compilation reports a Device schema type-table warning; isolated service compilation reproduces it | W21; preserve warnings-as-errors, investigate independently without editing official resources or claiming a full instance pass |
