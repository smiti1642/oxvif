...
| K30 / source-confirmed empty-token inconsistency | Explicit empty Create token can enter state that typed profile parsing rejects; missing/empty lookup and mutation policies disagree | W01/W10/W06; P2 documents retained behavior, raw/client state-after-error reproduction and explicit policy remain open |
| P2 / selected profile identity paired | Create/read/render/six binding paths now preserve decoded nonempty tokens through Media/PTZ workflows | W10 partial; other configuration fields, K30, adapter/replay keys, capacity and complete semantics remain open; see profile preflight |
