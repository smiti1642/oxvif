...
| W21 / M5 / PARTIAL | W20, D3 | Pinned offline tooling, 20 schema-free controls, seven independent-backend tests, and selected 44-operation profile/source/rate/encoder corpus export with explicit payload anchors | 148 instances pass locally after AM1, including 17 refusals (seven profile, three source, two rate, two encoder and three audio/metadata). Remaining operations and broader input/semantic coverage are not accepted |
| W22 / M5 / PARTIAL | W00 for inventory; W21 for schema job | Windows/Linux inventory, Xerces qualification, official-source compilation and selected profile/source/rate/encoder-instance validation gate package | Sources and corpus stay external with no uploaded artifacts. Selected corpus has 148 instances over 44 operations; AM1 hosted CI, whole-program instance coverage and release evidence check remain pending |
