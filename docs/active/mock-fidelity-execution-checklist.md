...
| W21 / M5 / PARTIAL | W20, D3 | Pinned offline tooling, 20 schema-free controls, seven independent-backend tests, and first 13-operation client/mock corpus export with explicit payload anchors | 30 instances: 28 pass, two missing-profile Faults fail (K03). All-source compilation passes; remaining operations and broader input/semantic coverage are not accepted |
| W22 / M5 / PARTIAL | W00 for inventory; W21 for schema job | Inventory and tooling passed prior hosted CI; Windows/Linux Xerces qualification and separate official-source compilation now gate package | Source-compilation jobs fetch only hash-pinned external resources and upload no artifacts. Full mock-instance job and release evidence check remain pending |
...
| K03 / reproduced for both DeleteProfile failures | Independent validation rejects flat `ter:NoProfile` in SOAP Code; client still exposes the first subcode contract | W05/W06; operation-specific structured mapping remains open; do not confuse successful client error parsing with valid wire format |
