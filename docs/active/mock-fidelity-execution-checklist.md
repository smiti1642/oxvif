...
| W21 / M5 / PARTIAL | W20, D3 | Pinned offline tooling, 20 schema-free controls, seven independent-backend tests, and selected 29-operation profile/source/rate/encoder corpus export with explicit payload anchors | 110 instances pass locally after VE1, including 14 refusals (seven profile, three source, two rate, two encoder). Remaining operations and broader input/semantic coverage are not accepted |
| W22 / M5 / PARTIAL | W00 for inventory; W21 for schema job | Windows/Linux inventory, Xerces qualification, official-source compilation and selected profile/source/rate/encoder-instance validation gate package | Sources and corpus stay external with no uploaded artifacts. Selected corpus has 110 instances over 29 operations; VE1 hosted CI, whole-program instance coverage and release evidence check remain pending |
...
Current progress: W00 is complete; 214 direct readers remain indexed, not accepted.
...
its dated evidence distinguishes local gates from hosted CI. Next is audio/metadata.
...
are implemented subgroups. **Next: W10 audio/metadata**, followed by other
