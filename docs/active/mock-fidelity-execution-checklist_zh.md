...
| W11／M2–M4／TODO | W01、W03–W06 | `services/ptz.rs`：selector、座標 attribute／space、configuration 子樹、preset／tour、auxiliary command | 兩個 head 具不同 space／capability；所有欄位效果、無效輸入 rollback；不虛構移動／時間保證 |
...
| W21／M5／PARTIAL | W20、D3 | 23 檔清單、固定 Python／Xerces 相依、離線工具、18 項無官方 schema 控制與五項必要獨立後端選型測試 | Xerces 無警告編譯全部 12 個 root；Python K21 保留診斷而非停用警告。Mock corpus／匯出與完整 SOAP／payload／Fault 驗收仍待完成 |
...
| K03／程式碼確認 | Fault builder 仍為平面，client 只公開一層 subcode | W05／W06；結構化 wire 及相容性證據 |
