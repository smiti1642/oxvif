...
| W19／M3、M6／PARTIAL | W03／W09 設計 | 內建 CreateProfile／DeleteProfile 使用私有 committed effect；加入選定跨服務讀取、HTTP、instance 及 chain 控制 | Binding 與其他 mutation、單獨 replay 政策、完整讀取依賴、正規化／key collision 及併發／callback 可見性仍待完成；不新增錄製設備機密 |
