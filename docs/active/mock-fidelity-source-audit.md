...
| --- | --- | --- |
| K27 | `canon::write_node` strips namespaces, collapses scalar whitespace and emits decoded text/attributes without escaping. Distinct identifiers or text/structure may share a key within one full Action. | W19/W10/W11: source-confirmed candidate; discriminating collision tests and a persisted-key migration design are required before token parsing/rendering closure. Not repaired by credential scrubbing. |
