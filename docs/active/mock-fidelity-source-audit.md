...
| W19 | `canon::canonicalize`: namespace-stripped DOM | Fixture keys, masking, replay, recording, adapter and invalidation; isolate recorded data, do not substitute synthetic strict parsing |
...
| --- | --- | --- |
| K27 — reproduced, not fixed | `mock_replay_key_gaps` proves six same-Action collisions: leading/repeated whitespace, field namespace, text versus structure, attribute boundary and ignored trailing root. Both requests replay the last response. | W19/W10/W11: passing known-gap test means reproduced; ordinary token and full-Action controls stay distinct. Persisted-key migration design remains required before token closure; credential scrubbing does not fix collisions. |
