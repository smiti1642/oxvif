...
| [WS-Discovery](#ws-discovery) | Multicast discovery and interface-aware APIs. |
| [Device and media](#device-service-methods) | Device, Media1, Media2, and audio operations. |
| [PTZ, imaging, and OSD](#ptz-methods) | Camera movement and image/overlay configuration APIs. |
| [Events and recording](#events-service-methods) | Pull events, recording, search, and replay. |
| [Health checks](#health-check-health-feature) | Diagnostic behavior, options, and report interpretation. |
| [Error handling](#error-handling) | Typed failures and handling patterns. |
| [Testing without a camera](#testing-without-a-real-camera) | Mock transport, mock server, and fault injection. |
| [Metamorph](#metamorph-metamorph--metamorph-server-features) | Clone, replay, compare, and adapt device behavior. |
| [Examples](#running-the-built-in-examples) | Commands for the repository's runnable examples. |
| [Implemented operations](#implemented-onvif-operations) | Link to the per-service coverage tables. |

---

## Architecture at a glance

oxvif is an async [ONVIF](https://www.onvif.org/) client for IP cameras
(Profiles S/T/G), covering discovery, device, media, PTZ, imaging, events, and
recording workflows.
...
**[OPERATIONS.md](OPERATIONS.md)** — per-service coverage tables for all nine
services plus WS-Discovery. Split out of this file to keep it navigable; the
method signatures and usage examples for each operation are in the sections
above.
...
See [CONTRIBUTING.md](CONTRIBUTING.md) for local gates and the mandatory
real-device redaction rules. Report vulnerabilities privately according to
[SECURITY.md](SECURITY.md), and follow the project
[code of conduct](CODE_OF_CONDUCT.md) in community spaces. Supported versions,
platforms, and compatibility-claim limits are defined in the
