...
//! ```
//!
//! All examples target a local camera. Adjust CAMERA_URL, USERNAME, and
//! PASSWORD at the top of this file before running.
...
const CAMERA_URL: &str = "http://192.168.4.50/onvif/device_service";
const USERNAME: &str = "admin";
const PASSWORD: &str = "aaaAAA111";
...
async fn main() {
    ...
    let result = match example.as_str() {
        "full-workflow"   => full_workflow().await,
        "device-info"     => device_info_example().await,
        "stream-uris"     => stream_uris().await,
        "error-handling"  => error_handling_example().await,
        _ => {
            print_help();
            return;
        }
...
async fn full_workflow() -> Result<(), OnvifError> {
    println!("=== Full workflow ===");
    println!("Connecting to {CAMERA_URL}");
    ...
    let client = OnvifClient::new(CAMERA_URL)
...
async fn device_info_example() -> Result<(), OnvifError> {
    println!("=== Device information ===");

    let info: DeviceInfo = match OnvifClient::new(CAMERA_URL).get_device_info().await {
        Ok(info) => info,
        Err(_) => {
            println!("(unauthenticated request failed — retrying with credentials)");
            OnvifClient::new(CAMERA_URL)
                .with_credentials(USERNAME, PASSWORD)
                .get_device_info()
                .await?
        }
...
async fn stream_uris() -> Result<(), OnvifError> {
    println!("=== Stream URIs ===");

    let client = OnvifClient::new(CAMERA_URL).with_credentials(USERNAME, PASSWORD);
...
async fn error_handling_example() -> Result<(), OnvifError> {
    ...
    match client.get_capabilities().await {
        ...
        Err(Err_::Transport(TransportError::Http(e))) => {
            ...
            eprintln!("Check that the camera is reachable at {CAMERA_URL}");
        }
